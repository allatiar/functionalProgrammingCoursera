...
class ListsSuite extends FunSuite {
  ...
  test("sum of a few numbers negative numbers") {
    assert(sum(List(1, 2, -3)) === 0)
  }
  test("sum of a few numbers negative numbers all") {
    assert(sum(List(-1, -2, -3)) === -6)
  }
  test("sum of a few numbers zeros") {
    assert(sum(List(1, 2, 0)) === 3)
  }
  test("sum of a few numbers zeros all") {
    assert(sum(List(0, 0, 0)) === 0)
  }
  test("sum of a few numbers empty lists") {
    intercept[IllegalArgumentException] {
      sum(List())
    }
  }
  test("sum of a few numbers 6") {
    assert(sum(List(1, 2, 3, 4, 5, 6)) === 21)
  }

  
  test("max of a few numbers lists with repeated elements") {
